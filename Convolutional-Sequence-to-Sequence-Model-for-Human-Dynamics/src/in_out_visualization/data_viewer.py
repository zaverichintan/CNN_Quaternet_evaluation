...
def main():
	...
	num = str(1)
	expmap_input = sio.loadmat('data/input_files/gt_xyz_walking_'+num+'.mat')['gt']
	expmap_output_gt = sio.loadmat('data/gt_lie_walking_'+num+'.mat')['gt']
	expmap_output_pred = sio.loadmat('data/prediction_lie_walking_'+num+'.mat')['prediction']

	nframes_gt, nframes_pred = expmap_output_gt.shape[0], expmap_output_pred.shape[0]
	nframes_in = expmap_input.shape[0]

	# # Put them together and revert the coordinate space
	expmap_all = revert_coordinate_space( np.vstack((expmap_output_gt, expmap_output_pred)), np.eye(3), np.zeros(3) )
	...
	for i in range( nframes_in ):
		xyz_in[i,:] = fkl( expmap_input[i,:], parent, offset, rotInd, expmapInd )
	...
	ob = viz_imple.Ax3DPose(ax)

	# Plot the input ground truth
	for i in range(nframes_in):
		print(i)
		ob.update(xyz_in[i, :], lcolor="#9b59c6", rcolor="#2ecc71")

		plt.show(block=False)
		fig.canvas.draw()
		# plt.pause(0.1)

		fig.savefig('./visualization/' + str('walking') + '/' + str(i) + '.png', dpi=fig.dpi)
