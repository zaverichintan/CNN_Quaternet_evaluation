...
def normalize_data( data, data_mean, data_std, dim_to_use, actions, one_hot ):
  ...
  if not one_hot:
    ...
    for key in data.keys():
      data_sel = np.reshape(np.copy(data[key]), [-1, 33, 3])
      theta = np.linalg.norm(data_sel, axis=2)
      data_sel = data_sel / np.expand_dims(theta + np.finfo(np.float).eps, 2)
      theta = np.mod(theta, np.pi * 2)
      data_sel = data_sel * np.expand_dims(theta, 2)
      data_sel = np.reshape(data_sel, [-1, 99])

      data_sel = (data_sel - np.expand_dims(data_mean, 0)) / np.expand_dims(data_std, 0)
      
      data_out[ key ] = data_sel[ :, dim_to_use ]
  else:
...
def normalization_stats(completeData):
  ...
  completeDataN = completeDataN / np.expand_dims(theta + np.finfo(np.float).eps, axis=2)
