...
# path='/media/data/zaveri/CNN/samples/'
path='/media/data/zaveri/CNN/samples/'
...
time = [1, 3, 7, 9, 13, 15, 17, 24]
errors=[]

print()
print("{0: <16} |".format("milliseconds"), end="")
for ms in [80, 160, 320, 400, 560, 640, 720, 1000]:
    print(" {0:5d} |".format(ms), end="")
print()

# with h5py.File(path+'CNNAdTrain_GANWEIGHT0.010000_Sampling0.950000WindownLength20-24000.h5', 'r' ) as h5f:
file_name = 'CNNAdTrain_GANWEIGHT0.010000_Sampling0.950000WindownLength20-24000.h5'
with h5py.File(path+file_name, 'r' ) as h5f:
    for action in actions:
        error=h5f['mean_{}_error'.format(action)][:]
        print("{0: <16} |".format(action), end="")
        for ms in time:
            print(" {0:.2f} |".format(error[ms]), end="")
        print()
        errors.append(error)
...
for ms in time:
    print(" {0:.2f} |".format(error_ave[ms]), end="")
print()
