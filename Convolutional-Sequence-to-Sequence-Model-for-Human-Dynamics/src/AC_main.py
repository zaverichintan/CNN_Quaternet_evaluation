...
# parse.add_argument("--output_length",help="how many previous frames should be used to predict the current frame",type=int,default=32)
...
parse.add_argument("--is_sampling",help="the current phase",type=bool,default=True)
parse.add_argument("--checkpoint", help="specify which model to load in", default=24000, type=int)
parse.add_argument("--dataset",help="choose a dataset for training",default="human3.6m",type=str)
# parse.add_argument("--dataset",help="choose a dataset for training",default="cmu",type=str)
...
model_name = 'CNNAdTrain_GANWEIGHT%f_Sampleing%fWindownLength%d' % (args.gan_loss_weight, args.use_sampling, args.window_length)

VAETrain = VAE.Autoencoder_gan.Autoenc_gan(encoder,
                                           decoder,
                                           discriminator,
                                           [None, 75, inputDimension, 1] if not args.is_sampling else [None,50+args.output_length,54,1],
...
with tf.Session() as sess:
    if not args.is_sampling:
        ...
        print("In is sampling")
        # VAETrain.Saver.restore(sess,'./Models/Models_kernel54/{}-{}'.format(model_name,args.checkpoint))
        VAETrain.Saver.restore(sess, '/media/data/zaveri/CNN/github_model/{}-{}'.format(model_name, args.checkpoint))
